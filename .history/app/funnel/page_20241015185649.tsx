...
const App = () => {
  ...
  const fetchChatHistory = async (projectCode: string, cursor: number | null = null, pageSize: number = 20) => {
    try {
      const params = {
        cursor,
        pageSize,
        asc: true,  // 假设我们想要按时间升序获取聊天历史
        projectCode
      };
      ...
      if (response && response.data) {
        ...
        setChatHistory(prevHistory => {
          // 如果是第一页（cursor 为 null），则替换整个历史
          // 否则，将新数据添加到现有历史中
          return cursor === null ? response.data.data : [...prevHistory, ...response.data.data];
        });

        console.log('Updated chat history:', response.data.data);

        // 返回是否还有更多数据和最后一条消息的 id（作为下一页的 cursor）
        return {
          hasMore: response.data.hasMore,
          nextCursor: response.data.data.length > 0 ? response.data.data[response.data.data.length - 1].id : null
        };
      } else {
