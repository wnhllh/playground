...
const App = () => {
  ...
  const fetchProjectData = async (projectCode: string) => {
    ...
    try {
      ...
      console.log('Preparing to call getArtifacts with params:', params);
      const response = await API.project.getArtifacts(params);
      console.log('Project data response:', response);

      // if (response && response.data) {
      //   console.log('Project artifacts:', response.data.artifacts);
      //   // 处理数据...
      ...
      //         case 'PROMPT':
      //           setPrompt(artifact.content || '');
      //           break;
      //         case 'CHAT_HISTORY':
      //           setChatHistory(JSON.parse(artifact.content || '[]'));
      //           break;
      //         case 'GENERATED_CODE':
      //           setGeneratedCodes(prevCodes => ({
      //             ...prevCodes,
      //             [artifact.meta]: artifact.content
      //           }));
      //           break;
      //         // 可以根据需要添加更多的 artifact 类型处理
      //       }
      //     });
      //   }
      // } else {
      //   console.error('Invalid response structure:', response);
      //   setError('获取项目数据失败：无效的响应结构');
      // }
    } catch (error) {
      console.error('获取项目数据失败:', error);
      if (error instanceof Error) {
        setError(`获取项目数据失败：${error.message}`);
      } else {
        setError('获取项目数据失败：未知错误');
      }
    }
  };
