...
const App = () => {
  ...
  const fetchProjectData = async (projectCode: string) => {
    try {
      ...
      if (response && response.data && response.data.artifacts && response.data.artifacts.length > 0) {
        ...
        // 使用 projectCode 作为 key 来更新 generatedCodes
        setGeneratedCodes(prevCodes => ({
          ...prevCodes,
          [projectCode]: artifact.content || ''
        }));
  ...
  // 在组件中添加一个状态来跟踪当前模式
  const [mode, setMode] = useState('create'); // 或者 'edit'

  // 在适当的地方根据 URL 参数或其他逻辑设置 mode
  useEffect(() => {
    // 例如，从 URL 参数中获取 mode
  ...
  // 确保我们有一个默认的 id 或 selectedId 可以使用
  const [selectedId, setSelectedId] = useState('default');

  // 在组件顶部定义 fallbackCode
