...
const App = () => {
  ...
  const fetchChatHistory = async (projectCode: string) => {
    console.log('fetchChatHistory called with projectCode:', projectCode);

    try {
      const params = {
        cursor: 0,  // 从第一页开始
        pageSize: 100,  // 假设我们一次加载100条消息，你可以根据需要调整
        asc: true,  // 按时间升序排列
        projectCode
      };
      ...
      if (response && response.data && response.data.data) {
        ...
        // 更新聊天历史状态
        setChatHistory(response.data.data.map(item => ({
          id: item.id.toString(),
          content: item.message,
          isUser: item.sender === 'USER'  // 假设 'USER' 表示用户消息，你可能需要根据实际情况调整
        })));

        // 如果需要，你可以处理 hasMore 字段来实现分页加载
