...
export const CodeGenerator = ({ id, defaultCode, projectCode }) => {
  ...
  // 修改 codeToDisplay 的获取逻辑
  const codeToDisplay =
    generatedCodes[id] ||            // 首先尝试使用 generatedCodes[id]
    generatedCodes[projectCode] ||   // 然后尝试使用 generatedCodes[projectCode]
    defaultCode ||                   // 如果都没有，使用 defaultCode
    fallbackCode;                    // 最后使用 fallbackCode
  ...
  useEffect(() => {
    ...
    console.log('codeToDisplay:', codeToDisplay);
  }, [generatedCodes, id, projectCode, defaultCode, codeToDisplay]);
